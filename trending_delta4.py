...
FILE_PATH = 'delta4_results_2020-01-02 12-22-20-554312.csv'  # UPDATE this for you, may need absolute path
...
class Plot:
    ...
    def update_source(self, attr, old, new):
        for source_key in [1, 2]:
            new_data = {key: [] for key in ['x', 'y', 'id', 'gamma_crit', 'file_name', 'gamma_index', 'daily_corr', 'dta']}
                    ...
                    if select_linac[source_key].value == 'All' or self.data['Radiation Dev'][i] == select_linac[source_key].value:
                        if end_date_picker.value > self.x[i] > start_date_picker.value:
                            ...
                            if 'Any' in active_gamma or gamma_crit in active_gamma:
                                # if 'Any' == select_beam_count[source_key].value or \
                                # int(self.data['Beam Count'][i]) == int(select_beam_count[source_key].value):
                                if 'Any' == select_energies[source_key].value or \
                                        self.data['Energy'][i] == select_energies[source_key].value:
...
class PlotControlChart:
    ...
    def __add_plot_data(self):
        self.plot_data = self.figure.circle('x', 'y', source=self.source['plot'],
                                            size=4, color='color', alpha='alpha')
    ...
    def update_plot(self):
        ...
        if select_y.value in ['Gamma-Index', 'DTA'] and ucl > 100:
            ucl = 100
        ...
        self.source['plot'].data = {'x': x, 'y': y, 'mrn': mrn, 'gamma_crit': gamma_crit, 'gamma_index': gamma_index,
                                    'daily_corr': daily_corr, 'dta': dta, 'color': color, 'alpha': alpha,
...
select_y = Select(title='Y-variable:', value='Dose Dev', options=y_options)
...
linacs.insert(0, 'All')
linacs.append('None')
select_linac = {key: Select(title='Linac %s:' % key, value='All', options=linacs, width=250) for key in [1, 2]}
...
# beam_counts = list(set(data['Beam Count']))
# beam_counts.sort()
# beam_counts = ['Any'] + [str(b) for b in beam_counts]
# select_beam_count = {key: Select(title='Beam Count %s:' % key, value='Any', options=beam_counts, width=250) for key in [1, 2]}
# select_beam_count[1].on_change('value', plot.update_source)
# select_beam_count[2].on_change('value', plot.update_source)

energies = list(set(data['Energy']))
energies.sort()
energies.insert(0, 'Any')
select_energies = {key: Select(title='Energy %s:' % key, value='Any', options=energies, width=250) for key in [1, 2]}
select_energies[1].on_change('value', plot.update_source)
select_energies[2].on_change('value', plot.update_source)
...
layout = column(row(select_y, select_linac[1], select_linac[2], avg_len_input, percentile_input),
                # row(select_beam_count[1], select_beam_count[2], select_energies[1], select_energies[2]),
